...
public class UserService {
    ...
    @Transactional
    public void updateLocalUser(User principalUser, UpdateRequest req) {
        // 0) 기존 연관 엔티티 필수 (신규 생성 안 함)
        User user = userRepository.findWithLocalAndDetail(principalUser.getId())
                .orElseThrow(() -> new IllegalStateException("유저 없음"));


        UserDetail  oldUd = user.getUserDetail();
        if (user.getLoginType() == LoginType.LOCAL) {
            LocalAccount la = user.getLocalAccount();
            if (notBlank(req.getPassword())) {
                la.updatePwd(passwordEncoder.encode(req.getPassword()));
            }
        }
        oldUd.updateUser(req.getName(),req.getPhone(),req.getGender(),req.getBirth(),req.getAddress());
